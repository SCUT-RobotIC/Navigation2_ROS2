...
void Preprocess::robosense_handler(const sensor_msgs::msg::PointCloud2::SharedPtr & msg)
{
  ...
  if (pl_orig.points[plsize - 1].timestamp > 0) {
    ...
    // test
    RCLCPP_WARN(rclcpp::get_logger("preprocess"), "PointCloud2 message does not contain timestamp, using angle-based time estimation!");
    given_offset_time = false;
    double yaw_first = atan2(pl_orig.points[0].y, pl_orig.points[0].x) * 57.29578;
    double yaw_end = yaw_first;
    int layer_first = pl_orig.points[0].ring;
...
void Preprocess::liosam_handler(const sensor_msgs::msg::PointCloud2::SharedPtr & msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();

  pcl::PointCloud<liosam_ros::Point> pl_orig;
  pcl::fromROSMsg(*msg, pl_orig);
  int plsize = pl_orig.points.size();
  if (plsize == 0) return;
  pl_surf.reserve(plsize);
