...
void exit_callback(const std_msgs::msg::Bool::SharedPtr msg)
{
  if (msg->data) {
    flg_exit = true;
    RCLCPP_WARN(LOGGER, "收到退出信号，设置flg_exit为True");
  }
}
...
bool reset_service_callback(
  const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
  std::shared_ptr<std_srvs::srv::Trigger::Response> response)
{
  RCLCPP_INFO(LOGGER, "收到重置服务请求，正在触发Point-LIO重置机制");
  flg_reset = true;
  response->success = true;
  response->message = "Point-LIO重置标志已设置，系统将在下一次循环中执行重置";
  return true;
}
...
int main(int argc, char ** argv)
{
  ...
  // 添加退出信号订阅者
  auto sub_exit_signal = nh->create_subscription<std_msgs::msg::Bool>(
    "point_lio_exit_signal", 10, exit_callback);
  RCLCPP_INFO(LOGGER, "已创建退出信号订阅者: ~/point_lio_exit_signal");
  ...
  // 创建重置服务
  auto reset_service = nh->create_service<std_srvs::srv::Trigger>(
    "point_lio_reset", reset_service_callback);
  RCLCPP_INFO(LOGGER, "Point-LIO重置服务已创建: ~/point_lio_reset");

  /******************************************mydebug************************************/
    // Define the publisher for debug LiDAR data
    auto pub_mydebug_lidar = nh->create_publisher<sensor_msgs::msg::PointCloud2>("debug_lidar", 1000);

    // Create a thread to publish LiDAR data when available
    std::thread lidar_publisher_thread([&]() {
      while (rclcpp::ok()) {
        if (!lidar_buffer_mydebug.empty()) {
          sensor_msgs::msg::PointCloud2 laserCloudMsg;
          // Lock to safely access the buffer
          std::unique_lock<std::mutex> lock(mtx_buffer);
          pcl::toROSMsg(*(lidar_buffer_mydebug.front()), laserCloudMsg);
          lidar_buffer_mydebug.pop_front();
          lock.unlock();
          
          // RCLCPP_WARN(nh->get_logger(), "msg point size: %ld,laserCloudMsg.height: %d,laserCloudMsg.width: %d  ", laserCloudMsg.data.size(), laserCloudMsg.height, laserCloudMsg.width);
          laserCloudMsg.header.stamp = get_ros_time(lidar_end_time);
          laserCloudMsg.header.frame_id = "velodyne";
          pub_mydebug_lidar->publish(laserCloudMsg);
        }
        else{
          // RCLCPP_WARN(nh->get_logger(), "buffer is empty, waiting for new data...");
        }
