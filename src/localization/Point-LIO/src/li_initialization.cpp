...
std::deque<PointCloudXYZI::Ptr> lidar_buffer_mydebug;
...
void standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::SharedPtr & msg)
{
  ...
  if ((lidar_type == VELO16 || lidar_type == OUST64 || lidar_type == HESAIxt32) && cut_frame_init) {
    RCLCPP_ERROR(rclcpp::get_logger("li_initialization"), "cut frame may not support");
    ...
    if (con_frame) {
      RCLCPP_ERROR(rclcpp::get_logger("li_initialization"), "con frame may not support");
      if (frame_ct == 0) {
      ...
      if (!ptr->points.empty()) {
        ...
        lidar_buffer_mydebug.emplace_back(ptr);
