...
enum LID_TYPE { AVIA = 1, VELO16, OUST64, HESAIxt32, ROBOSENSE,LIOSAM};  //{1, 2, 3, 4,5,6}
...
POINT_CLOUD_REGISTER_POINT_STRUCT(liosam_ros::Point,
                                 (float, x, x)
                                 (float, y, y)
                                 (float, z, z)
                                 (float, intensity, intensity)
...
//   EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ...
  void liosam_handler(const sensor_msgs::msg::PointCloud2::SharedPtr & msg);
